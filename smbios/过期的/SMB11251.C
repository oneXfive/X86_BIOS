...
void test_tp1(uchar *addr)
{
       uchar *entry =addr+*(addr+1);
       printf("entry:%x\n",entry);
        printf("addr:%x\n",*addr);
}
...
void find_type_end(unsigned char** addr)
{
      ...
      // if(**addr==1)printf("-%x-",*addr);
     // if(**addr==1)test_tp1(*addr);  
...
void print_type0(uchar *addr)
{
    ...
    printf("BIOS ROM Size %d\n",*(type0_entry_addr+0x9));
...
void print_type17(uchar *addr)
{
    ...
    size = *(type_entry_addr+0xc)&0x0fff;
    if(kb_or_mb)
    {
        printf("Size: %d MB\n",size);
    }
